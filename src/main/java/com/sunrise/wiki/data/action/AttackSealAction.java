...
public class AttackSealAction extends ActionParameter {
    ...
    public String localizedDetail(int level, Property property) {
        if(condition == Condition.hit)
            return I18N.getString("Make_s1_when_get_one_hit_by_the_caster_gain_one_mark_stack_max_s2_ID_s3_for_s4_sec",
            ...
            return I18N.getString("Make_s1_when_deal_damage_gain_one_mark_stack_max_s2_ID_s3_for_s4_sec",
                    targetParameter.buildTargetClause(),
                    Utils.roundDownDouble(actionValue1),
                    Utils.roundDownDouble(actionValue2),
                    buildExpression(level, durationValues, RoundingMode.UNNECESSARY, property));
        else if(condition == Condition.criticalHit && target == Target.owner)
            return I18N.getString("Make_s1_when_deal_critical_damage_gain_one_mark_stack_max_s2_ID_s3_for_s4_sec",
                    targetParameter.buildTargetClause(),
                    Utils.roundDownDouble(actionValue1),
                    Utils.roundDownDouble(actionValue2),
                    buildExpression(level, durationValues, RoundingMode.UNNECESSARY, property));
