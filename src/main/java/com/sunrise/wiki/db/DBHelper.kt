...
class DBHelper private constructor(var dbFilePath: String) {
    ...
    private fun getConnection(): Connection? {
        if (connection == null) connection = getConnection(dbFilePath)
        return connection
    }
...
//                        println(e.toString())
    ...
    private fun <T> getBeanByRaw(
        ...
        try {
            val cursor =
                statement?.executeQuery(sql) ?: return null
            val data: List<T>? = cursor2List(cursor, theClass)
            return if (data?.isNotEmpty() == true) data[0] else null
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    private fun getOne(sql: String?): String? {
        if (!dbFile.exists()) return null
        val cursor = statement?.executeQuery(sql)
        cursor?.next()
        return try {
    ...
    private fun getIntStringMap(
        ...
        return try {
            ...
            while (cursor?.next()!!) {
                result[cursor.getInt(cursor.findColumn(key))] = cursor.getString(cursor.findColumn(value))
            }
            cursor.close()
            result
        } catch (e: SQLException) {
     ...
     * 获取角色基础数据
     */
    fun getCharaInfo(unitId: Int): RawUnitBasic? {
        return getBeanByRaw(
            """
                SELECT ud.unit_id
                ,ud.unit_name
    ...
    fun getCNUnitSkillData(unitId: Int): RawUnitSkillDataCN? {
        //日服国服区分，数据库表结构存在差异
        if ("JP" == Statics.USER_LOC) {
            return getBeanByRaw<RawUnitSkillDataCN>(
                """
                SELECT * 
                FROM unit_skill_data 
                WHERE unit_id=$unitId 
                """,
                RawUnitSkillDataCN::class.java
            )
        } else {
            return getBeanByRaw<RawUnitSkillDataCN>(
                """
     ...
     * @return
     */
    fun getCNEnemy(enemyIdList: List<Int>): List<RawEnemyCN>? {
    ...
    fun getEquipmentPiece(): List<RawEquipmentPiece>? {
        return getBeanListByRaw(
            " SELECT * FROM equipment_data WHERE equipment_id >= 113000 ",
